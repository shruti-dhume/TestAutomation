...
public class LeavePage {
	...
	/*
	@FindBy(xpath = "//input[contains(@id,'Save') and @type = 'button']")
	WebElement Save
	
	
	
	 * @FindBy(id = "entitlements_leave_type") WebElement Leave_Type;
	...
	*/
	...
	@Step("Add Leave Entitlement")
	public void add_leave_entitlement() throws Exception
	{
		System.out.println("In Add Leave Entilement Method");
		Boolean isOptionSelected = null;
		//Emp_Name.sendKeys("Steven Edwards");
		Emp_Name.sendKeys("Russel");
		Thread.sleep(5000);
		String optionToSelect = "Russel Hamilton";
		...
		Leave_Type.selectByVisibleText("Paternity US");
		Select 	Leave_Period = new Select(driver.findElement(By.xpath("//select[@id='period']")));
		Leave_Period.selectByIndex(5);
		Entitlement_input.sendKeys("2");
		...
		System.out.println("Number of frames on the page :" + ele.size()); 
		...
		{ 
				...
				String FrameId = el.getAttribute("id"); 
		 ...
		 * 
		 * // Handling popup alert Alert alert = driver.switchTo().alert();
		 * 
	...
	public void verify_entitlement_submitted()
	{
		...
		/*
		 * Iterator <WebElement> iterator = Added_Entitlement.iterator();
		 * while(iterator.hasNext())
		 * System.out.println("Values in the result table are:" +
